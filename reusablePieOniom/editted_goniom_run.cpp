...
// this function would run "cmd", and return the output of cmd. "input" is the input file which cmd will use to run. "outname" is the name of the output file. input and output should be consistant with "cmd".
  ...
  string res;
  const char *cmdstr;
  cmd="goniom_gdvEditted.tcsh "+outname +cmd; //Call to Gaussian program is handled in the goniom_gdv.tcsh, which should be located in user's PATH. 
...
void  goniom_run(){
  ...
  clock_t begin;
  clock_t end;
  double elapsed_secs = 0.0; // CLOCKS_PER_SEC;
  ////////////////////////////////////////////////////////////////////////////////////
  ...
  if(mpi_id==0) {
    if(!ifAdmp) { //check for BOMD
      ...
      begin = clock();
      /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
      if((mulchg[0] != 0)||(ifCharge == 1))  mulchg=calc_chg(sub_subsys[0]) + "  1"+'\n';
  ...
  cout << "Time elapsed in Full system calculation is " << elapsed_secs << endl;
  /////////////////////////////////////////////////////////////////////////////////////
  ...
  if(mpi_id>ad){
    ...
    for(int i=mpi_id-ad;i<sub_subsys.size();i+=2*mpi_np) {
      ...
      if(ii<sub_subsys.size()){
        ...
        pid=fork();}
        if(pid==0) i=i+mpi_np; 
        if((mulchg[0] != 0)||(ifCharge == 1))  mulchg=calc_chg(sub_subsys[i]) + "  1"+'\n';
        if(!ifCharge) {
          f1=high+'\n'+title+'\n'+mulchg+sub_subsys[i]+'\n'+'\n';
          f2=low+'\n'+title+'\n'+mulchg+sub_subsys[i]+'\n'+'\n';
        }
  ...
  elapsed_secs = double(end - begin) / CLOCKS_PER_SEC;
  cout << "Time elapsed in all the fragments calculations  is " << elapsed_secs << endl;
  /////////////////////////////////////////////////////////////////////////////////////
  ...
  elapsed_secs = double(end - begin) / CLOCKS_PER_SEC;
  cout << "Time elapsed in mpiReduce function  is " << elapsed_secs << "with mpi_id as " << mpi_id << endl;
  /////////////////////////////////////////////////////////////////////////////////////
}
